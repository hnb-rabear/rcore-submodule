...
public class EnvSettingEditor : UnityEditor.Editor
{
	...
	private void DrawSettingsProfiles()
	{
		...
		EditorHelper.BoxVertical("Project Settings" + (m_previewingProfiles ? " Preview" : ""), () =>
		{
			...
			{
				m_profileCollections.profiles.Insert(0, new EnvSetting.Profile()
				{
					name = "do_not_remove",
				});
			}
	...
	private void DrawProfilesSelection()
	{
		EditorHelper.BoxVertical(() =>
		{
			var profiles = m_profileCollections.profiles ?? new List<EnvSetting.Profile>();
	...
	[MenuItem("RCore/Tools/Open Env Setting %_&_j")]
